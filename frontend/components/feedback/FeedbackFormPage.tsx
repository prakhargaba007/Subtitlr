...
export type FeedbackFormPageProps = {
  apiPath?: string;
...
export default function FeedbackFormPage({
  ...
  initialType = "general",
  badgeText = "Share your thoughts",
  badgeIcon = "feedback",
  title = "We'd love your feedback",
  description = "Every suggestion helps us build a better Kili. Tell us what you think — good, bad, or anywhere in between.",
  submitLabel = "Send Feedback",
  successTitle = "Thank you! 🎉",
  successDescription = "Your feedback has been received. We'll send a confirmation to your email and will use your input to improve Kili.",
  footerNote = "We read every submission personally. Your email is only used to follow up if needed.",
}: FeedbackFormPageProps) {
  const initial = useMemo<FormState>(
    () => ({ name: "", email: "", type: initialType, rating: 5, message: "" }),
    [initialType],
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMsg("");
    try {
      await axiosInstance.post(apiPath, form);
      setStatus("success");
      setForm(initial);
    } catch (err: unknown) {
      const msg =
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        <div className="text-center mb-16">
          <div className="inline-flex items-center space-x-2 bg-primary/5 px-4 py-1.5 rounded-full mb-8">
            <span className="material-symbols-outlined text-base leading-none text-primary">{badgeIcon}</span>
            <span className="text-xs font-headline font-bold uppercase tracking-widest text-primary">{badgeText}</span>
          </div>

          <h1 className="font-headline text-h2 md:text-h1 leading-[1.1] font-bold text-on-surface mb-6 tracking-tight">
            {title}
          </h1>
          <p className="text-on-surface-variant text-body-lg max-w-2xl mx-auto leading-relaxed font-light">{description}</p>
        </div>
        ...
        {status === "success" ? (
          <div className="bg-surface-container-lowest p-10 rounded-4xl border border-outline-variant/10 shadow-xl text-center space-y-6">
            ...
            <h2 className="text-h3 font-bold font-headline">{successTitle}</h2>
            <p className="text-on-surface-variant text-body max-w-md mx-auto">{successDescription}</p>
            <button
              type="button"
              onClick={() => {
                setStatus("idle");
          ...
          >
            <div>
              <label className="block text-body font-headline font-bold text-on-surface mb-4">What kind of feedback is this?</label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {FEEDBACK_TYPES.map((t) => (
                  ...
                  >
                    <span className="material-symbols-outlined text-2xl mt-0.5 shrink-0 transition-colors text-primary">{t.icon}</span>
                    <div>
                      <p className={["text-body font-headline font-bold", form.type === t.id ? "text-primary" : "text-on-surface"].join(" ")}>
                        {t.label}
                      </p>
                      <p className={["text-xs mt-1", form.type === t.id ? "text-primary opacity-80" : "text-on-surface-variant"].join(" ")}>
                        {t.desc}
                      </p>
                    </div>
                    {form.type === t.id && <span className="absolute top-4 right-4 w-2.5 h-2.5 rounded-full bg-primary" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="block text-body font-headline font-bold text-on-surface mb-3">How would you rate your experience?</label>
                  ...
                  >
                    <span
                      className={[
                        ...
                        star <= form.rating ? "text-amber-500 scale-110" : "text-outline/40 group-hover:text-amber-400",
                      ].join(" ")}
            ...
            <p className="text-center text-xs text-on-surface-variant max-w-sm mx-auto pt-2">{footerNote}</p>
          </form>
        )}
      </div>
    </main>
