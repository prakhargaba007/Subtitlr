...
exports.submitFeedback = async (req, res, next) => {
  try {
    ...
    if (!String(email || "").trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ...
    const typeLabel = { bug: "🐛 Bug Report", feature: "💡 Feature Request", general: "💬 General", praise: "🌟 Praise" }[feedbackType];
    const ratingText = feedbackRating ? `${"⭐".repeat(feedbackRating)} (${feedbackRating}/5)` : "Not rated";

    const ownerHtml = `
      <h2 style="font-family:sans-serif">New Feedback — Subtitlr</h2>
    ...
    const userHtml = `
      <div style="max-width:520px;margin:auto;padding:24px;font-family:'Segoe UI',Arial,sans-serif;background:#0f1117;border-radius:12px">
        ...
        <p style="color:#9aa3ad;font-size:13px">We read every piece of feedback and use it to make Subtitlr better. 💜</p>
        <p style="color:#9aa3ad;font-size:13px">— The Subtitlr Team</p>
      </div>
    `;

    // Fire-and-forget emails (don't block the response)
    res.status(201).json({ success: true, id: doc._id, message: "Feedback submitted. Thank you!" });

    try {
      await sendEmail(ownerEmail, `[Subtitlr Feedback] ${typeLabel} from ${name}`, message, ownerHtml, [], 3, "Subtitlr");
      await sendEmail(email, "Thanks for your feedback — Subtitlr", message, userHtml, [], 3, "Subtitlr");
    } catch (emailErr) {
