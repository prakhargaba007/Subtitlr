...
exports.getMySubscription = async (req, res, next) => {
  try {
    const rows = await UserSubscription.find({ user: req.userId })
      .sort({ updatedAt: -1 })
      .populate("planCatalog", "key displayName interval creditsPerPeriod version")
      .lean();
...
exports.createDodoCheckoutSession = async (req, res, next) => {
  try {
    ...
    const plan = (
      await PlanCatalog.find({ key: planKey, isActivePublic: true }).sort({ version: -1 }).limit(1).lean()
    )[0];
    if (!plan) {
    ...
    if (!client) {
      return res.status(503).json({ message: "DODO_PAYMENTS_API_KEY is not configured." });
    }

    const returnUrl = String(process.env.DODO_CHECKOUT_RETURN_URL || "").trim() || null;

    const session = await client.checkoutSessions.create({
      product_cart: [{ product_id: plan.dodoProductId, quantity: 1 }],
      customer: {
        email: String(user.email).trim(),
        name: String(user.name || user.userName || "Customer").trim() || "Customer",
      },
