...
/**
 * When `req.body.s3Key` is set (JSON start), validate HEAD + stream object to a temp file,
 * then set `req.dubbingTmpProbeFile`, `req.dubbingPresignedS3Key`, and `req.file` metadata
 * for `checkDubbingLimits` / `startDubbingJob`.
 * If `s3Key` is absent, calls `next()` so multer can handle multipart.
 */
async function prepareDubbingStartFromS3(req, res, next) {
  const rawKey =
  ...
  const prefix = `dubbing/${req.userId}/`;
  ...
  try {
    ...
    dest = path.join(os.tmpdir(), `dub_s3_${uuidv4()}${ext}`);
    await downloadS3ObjectToFile(rawKey, dest);

    req.dubbingTmpProbeFile = dest;
    req.dubbingPresignedS3Key = rawKey;
    req.file = {
      fieldname: "file",
